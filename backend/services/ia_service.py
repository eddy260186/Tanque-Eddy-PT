        ...
        if res.data:
            ...
            if compro_guia:
                ...
                if ultima_recarga is None or (hoy - ultima_recarga).days >= 30:
                    ...
                    logger.info(f"🔄 Créditos renovados: {email_usuario}")
...
def procesar_consulta_ia_con_memoria(alumno_id: str, mensaje_alumno: str) -> str:
    ...
    try:
        ...
        conteo_hoy = supabase.table("historial_ia").select("id", count="exact").eq("alumno_id", alumno_id).eq("rol_mensaje", "user").gte("fecha_creacion", f"{fecha_hoy}T00:00:00Z").execute()

        if (conteo_hoy.count or 0) >= 10:

            return "⚠️ Alcanzaste el límite diario de consultas IA. Mañana seguimos entrenando 💪"
        ...
        objetivo = atleta_data.get("objetivo_principal", "Mantenerse saludable")
...
"""
        ...
        if not respuesta_texto:
            ...
            respuesta_texto = "⚠️ No pude generar respuesta en este momento."

        respuesta_texto = str(respuesta_texto).strip()
        ...
        logger.error(f"❌ Error crítico IA: {str(e)}")

        return "👋 Estoy actualizando mis servidores IA. Probá nuevamente en unos minutos."

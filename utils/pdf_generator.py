...
# ==========================================================
# EDDY PERSONAL TRAINER ELITE - MOTOR PDF BLINDADO V9.0
# ==========================================================

def build_pdf_v60_7(d, grafico_b64, ruta_img, gen):

    # ======================================================
    # 1. DETECCIÓN DE GÉNERO Y COLORES
    # ======================================================
    is_f = (gen == "f")
    ...
    if ruta_final and os.path.exists(ruta_final):
        ...
        logo_html = f'<img src="data:image/png;base64,{logo_b64}" class="logo-main">'
        logo_chico_html = f'<img src="data:image/png;base64,{logo_b64}" class="logo-small">'
    ...
    if os.path.exists("qr_code.png"):
        ...
        qr_html = f'<div class="qr" style="background:#111; color:{c_accent}; font-size:8px; display:flex; align-items:center; justify-content:center;">QR</div>'

    # ======================================================
    # 4. HTML CABECERA Y PORTADA
    # ======================================================
    html = f"""
    <html>
    <head>
    ...
    *{{ box-sizing:border-box; }}
    ...
    body:before{{
        content:''; position:fixed; width:800px; height:800px; top:-300px; left:-200px;
        background: radial-gradient(circle, {c_dark}, transparent 70%); filter:blur(90px); z-index:-1;
    }}
    ...
    .logo-main{{ height:360px; filter: drop-shadow(0 0 15px {c_accent}) drop-shadow(0 0 35px {c_dark}) brightness(1.08) contrast(1.08); }}
    .cover-title{{
        font-family:'Bebas Neue'; font-size:74px; letter-spacing:8px; margin-top:-15px;
    ...
    .cover-sub{{ color:{c_soft}; letter-spacing:4px; font-size:11px; margin-top:-10px; }}
    .badge{{
        margin-top:30px; padding:12px 35px; border-radius:50px;
        background: linear-gradient(145deg, rgba(255,255,255,0.05), rgba(255,255,255,0.01));
        border: 1px solid rgba(255,255,255,0.08); box-shadow: 0 0 20px {c_dark}; font-weight:700; letter-spacing:4px;
    }}
    .athlete{{ margin-top:40px; width:70%; border-top: 1px solid rgba(255,255,255,0.08); border-bottom: 1px solid rgba(255,255,255,0.08); padding:25px 0; }}
    .athlete-name{{
        font-family:'Bebas Neue'; font-size:52px; letter-spacing:3px; margin:0;
        background: linear-gradient(180deg, #FFF3B0, {c_bright}, {c_accent});
        -webkit-background-clip:text; -webkit-text-fill-color:transparent;
    }}

    .page{{ padding:40px; page-break-before:always; }}
    .header{{ display:flex; justify-content:space-between; align-items:end; border-bottom: 1px solid rgba(255,255,255,0.08); padding-bottom:15px; margin-bottom:25px; }}
    .header-title{{
        font-family:'Bebas Neue'; font-size:34px; letter-spacing:3px;
        background: linear-gradient(180deg, #FFF3B0, {c_bright}, {c_accent});
        -webkit-background-clip:text; -webkit-text-fill-color:transparent;
    }}
    .logo-small{{ height:60px; filter: drop-shadow(0 0 10px {c_dark}); }}

    .grid-4 {{ width: 100%; margin-bottom: 20px; }}
    ...
    .card{{
        ...
        border: 1px solid rgba(255,255,255,0.06); border-top: 3px solid {c_accent};
        border-radius:18px; padding:22px;
        box-shadow: 0 10px 25px rgba(0,0,0,0.8), 0 0 18px {c_dark}; position:relative; overflow:hidden;
    }}
    .card:before{{
    ...
    .value{{
        ...
        -webkit-background-clip:text; -webkit-text-fill-color:transparent; text-shadow: 0 4px 12px rgba(0,0,0,0.7);
    }}
    .bar-bg{{ width:100%; height:6px; background:#1A1A1A; border-radius:20px; margin-top:15px; overflow:hidden; }}
    .bar-fill{{
        height:100%; border-radius:20px;
        background: linear-gradient(90deg, #FFF6CC, {c_bright}, {c_accent}, #7A5A00); box-shadow: 0 0 15px {c_dark};
    }}

    .list-card{{
        background: linear-gradient(145deg, rgba(255,255,255,0.03), rgba(255,255,255,0.01));
        border-left: 4px solid {c_accent}; border-radius:16px; padding:22px; margin-bottom:18px; box-shadow: 0 8px 20px rgba(0,0,0,0.8);
    }}
    .list-title{{ font-family:'Bebas Neue'; font-size:24px; letter-spacing:2px; margin-bottom:12px; color:{c_bright}; }}
    .item{{ padding:10px 0; border-bottom: 1px solid rgba(255,255,255,0.04); font-size:11px; color: #E0E0E0; }}
    .data-table {{ width: 100%; border-collapse: collapse; }}
    .data-table td {{ padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.04); font-size: 11px; color: #E0E0E0; }}
    .graph{{ width:100%; border-radius:16px; border: 1px solid rgba(255,255,255,0.08); box-shadow: 0 0 25px rgba(0,0,0,0.8); }}
    .footer{{ margin-top:40px; display:flex; justify-content:space-between; align-items:end; border-top: 1px solid rgba(255,255,255,0.08); padding-top:18px; }}
    .signature{{ font-family:'Great Vibes'; font-size:34px; color:{c_soft}; }}
    .qr{{ width:60px; height:60px; background:white; padding:3px; border-radius:6px; border: 1px solid {c_accent}; box-shadow: 0 0 18px rgba(0,0,0,0.7); }}
    </style>
    </head>
    <body>
    <div class="cover">

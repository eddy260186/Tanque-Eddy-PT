...
def calcular_tmb(genero_simplificado, peso, estatura_cm, edad):
    ...
    """
    # Convertir el género simplificado a 'hombre' o 'mujer' para la fórmula
    # 'm' -> 'hombre', 'f' -> 'mujer'
...
def registrar_usuario():
    ...
    # Género simplificado: 'm' para masculino, 'f' para femenino
    ...
    estatura_cm = solicitar_numero("Estatura en cm (ej: 175): ", tipo=float, minimo=50.0, maximo=250.0) # Límite de estatura razonable
    ...
    calorias_objetivo = 0
    objetivo_texto = ""
    kilos_a_cambiar = 0 # Variable para almacenar kilos a bajar/ganar si aplica
    ...
    if opcion_objetivo == '1': # Pérdida de Peso
        ...
        min_calorias = 1500 if genero.lower() == 'm' else 1200 # Usamos 'm' para masculino
    ...
    if opcion_objetivo == '1':
        print(f"   Para perder {kilos_a_cambiar:.1f} kg, tu consumo calórico diario objetivo es: {calorias_objetivo:.0f} kcal/día.")
    ...
    for comida, porcentaje in distribucion_comidas.items():
        ...
        print("-" * 25) # Separador más corto para las comidas

    print("="*60)

    # --- Guardar datos ---
    try:
        # Guardar en archivo de texto simple
        with open("registros_nutricionales.txt", "a", encoding='utf-8') as archivo:
            ...
            # Guarda el género completo para mayor claridad en el archivo
            ...
            # Añadir la guía de alimentación al archivo
            archivo.write("\n--- Guía de Alimentación por Comida ---\n")
            for comida, porcentaje in distribucion_comidas.items():
                calorias_comida = calorias_objetivo * porcentaje
                proteinas_comida = proteinas * porcentaje
